...
echo -----------------------------------
echo -en '\033[47;31m'"\033[1mChecking xlrd installation\033[0m" 
tput sgr0
echo
xlrdName=`/usr/local/bin/pip2.7 freeze | grep xlrd`
if [ -z $xlrdName ]; then
    echo Xlrd not installed... installing
    /usr/local/bin/pip2.7 install xlrd
else
    echo Xlrd already installed
fi
...
tput sgr0
echo
curl -sL https://github.com/milegroup/gasatad/raw/gh-pages/packages/GASATaD_1_0.app.tgz -o $TMPDIR/GASATaD_1_0.app.tgz
tar xfz $TMPDIR/GASATaD_1_0.app.tgz -C $TMPDIR
rm -rf /Applications/GASATaD.app
mv $TMPDIR/GASATaD.app /Applications
echo GASATaD ver. 1.0 has been added to the Applications folder
